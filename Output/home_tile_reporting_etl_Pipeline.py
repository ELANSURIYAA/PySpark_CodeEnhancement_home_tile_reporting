...
Date: 
Description: PySpark ETL pipeline updated to enrich home tile reporting with tile category from metadata table (SCRUM-7567)
===============================================================================
Functional Description:
    ...
    - Enriches reporting outputs with tile_category from metadata
    - Loads aggregated results into:
        • TARGET_HOME_TILE_DAILY_SUMMARY (now with tile_category)
...
1.1.0       <today>       Ascendion AAVA    SCRUM-7567: Added tile metadata enrichment (# [ADDED] tile_category)
-------------------------------------------------------------------------------
"""
...
SOURCE_TILE_METADATA = "analytics_db.SOURCE_TILE_METADATA"  # [ADDED] SCRUM-7567: Metadata table config
...
spark = (
    SparkSession.getActiveSession() # [MODIFIED] Use Spark Connect compatible session
)
...
# READ METADATA TABLE
# ------------------------------------------------------------------------------
df_metadata = spark.table(SOURCE_TILE_METADATA).filter(F.col("is_active") == True) # [ADDED] SCRUM-7567: Read only active tiles
metadata_count = df_metadata.count()
print(f"Loaded {metadata_count} active tile metadata records") # [ADDED] Logging for metadata
...
# ------------------------------------------------------------------------------
# JOIN METADATA FOR TILE CATEGORY ENRICHMENT
# ------------------------------------------------------------------------------
df_daily_summary = (
    df_tile_agg.join(df_inter_agg, "tile_id", "outer")
    .join(df_metadata.select("tile_id", "tile_category"), "tile_id", "left") # [ADDED] Join with metadata
    .withColumn("date", F.lit(PROCESS_DATE))
    .select(
        "date",
        "tile_id",
        F.coalesce("tile_category", F.lit("UNKNOWN")).alias("tile_category"), # [ADDED] tile_category with default
...
unknown_tiles = df_daily_summary.filter(F.col("tile_category") == "UNKNOWN").count() # [ADDED] Validation for unmapped tiles
if unknown_tiles > 0:
    print(f"Warning: {unknown_tiles} tiles have no metadata mapping") # [ADDED] Logging for missing metadata
...
# ------------------------------------------------------------------------------
# SCHEMA VALIDATION FOR DAILY SUMMARY
# ------------------------------------------------------------------------------
expected_columns = [
...
] # [ADDED] SCRUM-7567: Validate new column
actual_columns = df_daily_summary.columns
assert set(expected_columns).issubset(set(actual_columns)), "Schema validation failed for TARGET_HOME_TILE_DAILY_SUMMARY" # [ADDED] Assertion
...
# [DEPRECATED] Old df_daily_summary logic without tile_category
# df_daily_summary = (
#     df_tile_agg.join(df_inter_agg, "tile_id", "outer")
#     .withColumn("date", F.lit(PROCESS_DATE))
#     .select(
